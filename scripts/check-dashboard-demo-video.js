...
async function main() {
  const videoPath = path.resolve(__dirname, "..", "docs", "video", "think-dashboard-demo-seguro.webm");
  ...
  const metadata = await page.evaluate(async () => {
    ...
    video.currentTime = Math.min(20, video.duration / 2);
  ...
  await page.screenshot({
    path: path.resolve(__dirname, "..", "tmp", "demo-video-check.png"),
    fullPage: true,
  });
